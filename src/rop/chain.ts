...
export interface ExportableEmission {
  planId: number;
  strategy: string;
  shape: string;
  gadgets: Array<{
...
export interface ExportableSynthesis {
  planId: number;
  strategy: string;
  shape: string;
  entryPath: string;
  status: string;
  slots: Array<{ offset: number; role: string; step: ChainStep }>;
  placeholders: string[];
  violations: string[];
}

export function formatEmitPython(emit: ExportableEmission): string[] {
  const lines: string[] = [
...
export function formatSynthesisPython(synth: ExportableSynthesis): string[] {
  const lines: string[] = [
  ...
  for (const slot of synth.slots) {
    const word = slot.step.kind === "gadget"
...
export function formatExportPython(
  emit: ExportableEmission,
  synth?: ExportableSynthesis,
): string[] {
  const lines: string[] = [
    "#!/usr/bin/env python3",
