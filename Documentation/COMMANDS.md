...
| `egghunter` | `dx @$osed().egghunter({ tag?, mode?, wow64?, badchars? })` | `dx @$osed().egghunter({ tag: "W00T", badchars: [0, 0x0a] })` | Generates egghunter shellcode (hex + Python). Mode `ntaccess` (32 bytes, INT 0x2E syscall) or `seh` (70 bytes, SEH-based — no syscall dependency, contains null bytes). Reports any stub byte that collides with the supplied bad-character list. |
...
| `rop.chain_vp` | `dx @$osed().rop.chain_vp({ mode?, virtualProtect?, retGadget?, returnAddress?, dwSize?, writable?, flNewProtect? })` | `dx @$osed().rop.chain_vp({ virtualProtect: 0x7C801AD0 })` | VirtualProtect PUSHAD chain built from the loaded corpus. Default `mode` is `ret-slide`: `EDI = ret`, `ESI = VirtualProtect`, saved ESP becomes `lpAddress`, `EBX = dwSize` (default `0x201`), `EDX = 0x40`, and `ECX = WRITABLE`. `mode: "direct"` is available but warns that saved ESP becomes `dwSize`. Read-only. |
| `rop.chain_wpm` | `dx @$osed().rop.chain_wpm({ writeProcessMemory?, returnAddress?, lpBuffer?, nSize?, writable? })` | `dx @$osed().rop.chain_wpm({ writeProcessMemory: 0x7C802213 })` | Constrained WriteProcessMemory PUSHAD chain. Hard-codes `hProcess = 0xFFFFFFFF`; leaves `WRITEPROCESSMEMORY`, `RETURN_ADDR`, `LP_BUFFER`, `NSIZE`, `WRITABLE` as placeholders when not supplied. Warns that direct PUSHAD uses saved ESP as `lpBaseAddress`, so the chain is only a DEP bypass if that saved ESP is already an executable destination. Read-only. |
| `rop.chain_va` | `dx @$osed().rop.chain_va({ virtualAlloc?, returnAddress?, lpAddress?, flAllocationType?, flProtect? })` | `dx @$osed().rop.chain_va({ virtualAlloc: 0x7C809AE1 })` | Constrained VirtualAlloc PUSHAD chain. Defaults `flAllocationType = 0x1000` and `flProtect = 0x40`; warns that direct PUSHAD uses saved ESP as `dwSize`, which must be verified or replaced with a different chain shape. Read-only. |
| `rop.frame_vp` | `dx @$osed().rop.frame_vp({ virtualProtect?, returnAddress?, lpAddress?, dwSize?, flNewProtect?, writable?, badchars? })` | `dx @$osed().rop.frame_vp({ virtualProtect: 0x7C801AD0, badchars: [0, 10, 13] })` | Flat VirtualProtect stdcall frame: `[VirtualProtect][return][lpAddress][dwSize][flNewProtect][lpflOldProtect]`. Requires no loaded ROP corpus and no gadgets. Defaults `dwSize = 0x201`, `flNewProtect = 0x40`; checks every concrete dword against `badchars` and warns that placeholders must be validated after resolution. |
| `rop.frame_wpm` | `dx @$osed().rop.frame_wpm({ writeProcessMemory?, returnAddress?, hProcess?, lpBaseAddress?, lpBuffer?, nSize?, writable?, badchars? })` | `dx @$osed().rop.frame_wpm({ writeProcessMemory: 0x7C802213, badchars: [0] })` | Flat WriteProcessMemory stdcall frame. Requires no loaded ROP corpus. Defaults `hProcess = 0xFFFFFFFF`; all other runtime values may be placeholders. Concrete values are checked against `badchars`. |
| `rop.frame_va` | `dx @$osed().rop.frame_va({ virtualAlloc?, returnAddress?, lpAddress?, dwSize?, flAllocationType?, flProtect?, badchars? })` | `dx @$osed().rop.frame_va({ virtualAlloc: 0x7C809AE1, badchars: [0, 10, 13] })` | Flat VirtualAlloc stdcall frame. Requires no loaded ROP corpus. Defaults `dwSize = 0x201`, `flAllocationType = 0x1000`, and `flProtect = 0x40`; concrete values are checked against `badchars`. |
