...
| `egghunter` | `dx @$osed().egghunter(tag?, mode?, wow64?, badchars?, syscall?)` | `dx @$osed().egghunter("W00T", "ntaccess", false, "00 0A", 0x1c9)` | Generates egghunter shellcode (hex + Python). Mode `ntaccess` (34 bytes, INT 0x2E syscall) or `seh` (70 bytes, SEH-based — no syscall dependency, contains null bytes). Optional `syscall` overrides the NtAccessCheckAndAuditAlarm syscall number (default 0x1C9 for Win10; get yours with `u ntdll!NtAccessCheckAndAuditAlarm L1`). Reports any stub byte that collides with the supplied bad-character list. |
